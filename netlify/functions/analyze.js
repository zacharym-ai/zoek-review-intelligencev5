exports.handler = async function (event) {
  ...
  const { reviews, reportDate, reportLabel } = body;
  ...
  function parseStars(text) {
    ...
    return { count: ratings.length || lines.length, avg, lines };
  }
  ...
  const npsBreakdown = {
    promoters: npsTotal > 0 ? Math.round((promoters/npsTotal)*100) : 0,
    passives: npsTotal > 0 ? Math.round((passives/npsTotal)*100) : 0,
    detractors: npsTotal > 0 ? Math.round((detractors/npsTotal)*100) : 0
  };
  ...
  try {
    const [summaryRaw, platRaw] = await Promise.all([
      callClaude("Zoek Marketing review analyst. Respond ONLY with raw JSON no markdown.\nDATE: " + (reportDate||"not specified") + " OVERALL: " + overallScore + "/5\n\n" + condensed + '\n\n{"whatsGoingRight":"<2 sentences>","areaOfConcern":"<2 sentences>","needsImmediateAttention":"<2 sentences or No critical issues>","overallSummary":"<3 sentences>","trend":"up|down|stable","trendReason":"<one sentence>","positiveTrendNotes":["<note>","<note>"],"negativeTrendNotes":["<note>","<note>"],"topPositiveKeywords":["<w>","<w>","<w>","<w>","<w>","<w>","<w>","<w>","<w>","<w>"],"topNegativeKeywords":["<w>","<w>","<w>","<w>","<w>","<w>","<w>","<w>","<w>","<w>"],"radar":{"areas":["Sales","Operations","Customer Success","Communications","Product/Delivery"],"strengths":[5,5,5,5,5],"improvements":[5,5,5,5,5],"leadershipRecs":[{"area":"<area>","rec":"<constructive rec>","department":"<dept>","priority":"high|medium|low"}]},"positiveShoutouts":[{"name":"<full name>","reason":"<10 words>","platform":"<platform>","reviewStars":5}],"negativeFlags":[{"name":"<full name>","reason":"<10 words>","platform":"<platform>","severity":"medium"}]}\nIMPORTANT: positiveShoutouts only if real name explicitly praised. negativeFlags only if real name explicitly criticized. Use [] if none.', 2500),
      callClaude("Zoek Marketing review analyst. Respond ONLY with raw JSON no markdown.\n\n" + platData + '\n\n{"google":' + (parsed.google.count>0?'{"summary":"<2 sentences>","positiveThemes":["<t>","<t>","<t>"],"negativeThemes":["<t>","<t>","<t>"],"positiveSnippets":["<quote under 15 words>","<quote>","<quote>"],"negativeSnippets":["<quote under 15 words>","<quote>","<quote>"],"actionItems":[{"action":"<action>","department":"Sales|Operations|Customer Success|Communications|Product/Delivery|Leadership"},{"action":"<action>","department":"<dept>"}]}':'null') + ',"wix":' + (parsed.wix.count>0?'{"summary":"<2 sentences>","positiveThemes":["<t>","<t>","<t>"],"negativeThemes":["<t>","<t>","<t>"],"positiveSnippets":["<quote>","<quote>","<quote>"],"negativeSnippets":["<quote>","<quote>","<quote>"],"actionItems":[{"action":"<action>","department":"<dept>"},{"action":"<action>","department":"<dept>"}]}':'null') + ',"trustpilot":' + (parsed.trustpilot.count>0?'{"summary":"<2 sentences>","positiveThemes":["<t>","<t>","<t>"],"negativeThemes":["<t>","<t>","<t>"],"positiveSnippets":["<quote>","<quote>","<quote>"],"negativeSnippets":["<quote>","<quote>","<quote>"],"actionItems":[{"action":"<action>","department":"<dept>"},{"action":"<action>","department":"<dept>"}]}':'null') + ',"bbb":' + (parsed.bbb.count>0?'{"summary":"<2 sentences>","positiveThemes":["<t>","<t>"],"negativeThemes":["<t>","<t>"],"positiveSnippets":["<quote>","<quote>","<quote>"],"negativeSnippets":["<quote>","<quote>","<quote>"],"actionItems":[{"action":"<action>","department":"<dept>"},{"action":"<action>","department":"<dept>"}]}':'null') + '}', 2500)
    ]);
    ...
    const report = {
      overallScore, reviewCounts, platformScores, npsScore, npsBreakdown,
    ...
    if (SUPA_URL && SUPA_KEY) {
      try {
        const label = reportLabel || reportDate || new Date().toISOString().split("T")[0];
        const date = reportDate || new Date().toISOString().split("T")[0];
        const saveRes = await fetch(SUPA_URL + "/rest/v1/reports", {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ label, date, report })
        });
        const saveText = await saveRes.text();
        console.log("Supabase save status:", saveRes.status, saveText.slice(0, 200));
      } catch(saveErr) {
        console.log("Supabase save error:", saveErr.message);
      }
    } else {
      console.log("Supabase not configured - SUPA_URL:", !!SUPA_URL, "SUPA_KEY:", !!SUPA_KEY);
    }
